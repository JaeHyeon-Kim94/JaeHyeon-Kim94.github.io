...
### 05-2. 재귀 알고리즘 분석
  ...
  ```java
    ...
    1 recur(4) 
    2   -> recur(3) -> print(4) -> recur(2)
    3                           -> recur(1) -> print(2) -> recur(0)
    4                           -> recur(0) -> print(1) -> recur(-1)
    5   -> recur(2) -> print(3) -> recur(1)
    6                           -> recur(0) -> print(1) -> recur(-1)
    7   -> recur(1) -> print(2) -> recur(0)
    8
    9   -> recur(0) -> print(1) -> recur(-1)
  ...
  - 재귀 알고리즘의 비재귀적 표현
    - 꼬리 재귀의 제거
      - recur 메서드의 꼬리에서 호출하는 recur(n-2)은 `n의 값을 n-2로 업데이트 하고 메서드의 시작 지점으로 돌아간다.`로 바꿀 수 있음.
        <br>이를 코드상으로 구현하면
        ```java
          static void recur(int n){
            while(n>0){
