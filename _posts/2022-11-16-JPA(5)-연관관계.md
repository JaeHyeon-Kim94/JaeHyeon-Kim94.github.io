...
- 외래 키를 통해 양방향 연관계가 관리되는 것처럼, 객체 양방향 연관관계 또한 외래키 필드 하나로 관리하기 위해 연관관계의 주인을 설정한다.
...
## 다양한 연관관계
 ...
 - 위의 예제에선 다대 일의 연관관계를 살펴보았다. 다음은 일대일, 다대다 연관관계.
...
```java
    public class Member{
        ...
        @ManyToMany
        @JoinTableTable( name = "member_product",
